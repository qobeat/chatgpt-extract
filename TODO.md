...
| Phase | Theme | Status |
|---|---|---|
| **1** | Benchmark validity & keep-vs-return re-decision | **DONE** |
| **2** | Catalog completeness & fidelity | **DONE** |
| **3** | Publish / redaction hardening + observability | **MOSTLY DONE** — remaining: `chatgpt-extract-catalog` observability integration + `VENDORED_FROM`; surface `GATE-PRIVACY` evidence into Project State |
| **4** | CLI / UX polish + packaging | **MOSTLY DONE** — remaining: `VENDORED_FROM` pinning of the catalog repo's vendored libs |

Completed work is also recorded in the durable surfaces:

- **`CHANGELOG.md`** — named, dated releases: **Semantics** (gpt ask/index +
  cross-sweep unify), **ADOS Geometry**, **Benchmark Validity & Model Bank**,
  **2.0.0**.
- **`REQUIREMENTS.md`** — satisfied requirements are tagged `[IMPLEMENTED]` with
  their verification; see §4 "Implemented in the current release".
- **`README.md`** — the command table + "Ask your chats" runbook; **`ESSAY.md`**
  states the durable thesis (`AUTHORITY_REF: project-geometry.json`).

### Shipped in 1.0 (see CHANGELOG → 1.0.0 "Semantics")

The roadmap items that defined the 1.0 release have landed (verified by
`pytest -q`):

- **Phase 2 — measured catalog coverage.** `COORD-C-COVERAGE` is now a measured
  observation in `gpt state` derived from the extract ledger
  (`seen`/`skipped`/`written`), not `unknown`.
- **Phase 3 — redaction hardening + gate-aware verdict.** `redact` broadened to
  JWTs, PEM private keys, and range-checked IPv4; `COORD-D-VERDICT` now carries
  `GATE-COVERAGE` / `GATE-SCHEMA` evidence so the score is gate-aware.
- **Phase 4 — `--json` everywhere.** All read-only/benchmark commands emit
  `--json` (including the new `gpt ask --json`).
- **Cross-sweep accuracy (FR-D3).** `--reference` is threaded through
  `gpt state --all`, so `COORD-B-ACCURACY` populates per workload.
- **Semantic ask enhancements (FR-Q).** `gpt ask --json`, `--rerank` lexical
  re-rank, chunk-level citations (char offsets), a stale-index warning, and a
  keyword-scan fallback when no index exists.
...
1. The catalog repo **vendors copies** of `paths.py`, `ulog.py`, `run_log.py`.
   ...
   two repos cannot silently drift. *(Phase 4 — remaining.)*
...
phase's exit criteria are its acceptance checks below, which map to specific
`FR-*` / `NFR-*` IDs in `REQUIREMENTS.md`.

---

## Phase 1 — Benchmark validity and the keep-vs-return re-decision  — STATUS: DONE
...
**Acceptance criteria:** `gpt metrics` reports completion / depth-on-success /
...
(FR-D2); `pytest -q` is green (NFR-Q1).

---

## Phase 2 — Catalog completeness and fidelity  — STATUS: DONE

**Outcome:** the catalog losslessly represents what is actually in the export, so
the benchmark workload (Phase 1) and any query (`skills/catalog-query`) operate on
complete data.
...
**Acceptance criteria:** extraction captures the previously-dropped content-types
...
## Phase 3 — Publish / redaction hardening and observability  — STATUS: MOSTLY DONE

**Remaining:** the `chatgpt-extract-catalog` observability integration (run
registry + `RUN_SUMMARY` + cross-run stats) and the `VENDORED_FROM` upstream-commit
markers (both depend on the separate private repo); surface `GATE-PRIVACY` evidence
from the publish/cloud path into Project State (today only `GATE-COVERAGE` /
`GATE-SCHEMA` are emitted). Active redaction, broadened patterns, publish-boundary
tests, and PII-free logs are **done**.
...
Reconciles the vendored-lib copies (sets up Phase 4's pinning).
...
semantics (Phase 2) or the benchmark metric (Phase 1).

**Actions and success conditions (priority order):**

1. **Redaction becomes an active transform (NFR-P2). — DONE**
...
2. **Broaden the patterns (NFR-P2). — DONE**
...
3. **Publish-boundary tests (NFR-P1). — DONE**
...
5. **Observability integration (Phase 3, NFR-Q4). — REMAINING**
...
**Acceptance criteria:** the publish path actively scrubs broadened PII (NFR-P2);
...
## Phase 4 — CLI / UX polish and packaging  — STATUS: MOSTLY DONE

**Remaining:** `VENDORED_FROM` pinning of the `chatgpt-extract-catalog` vendored
libs (depends on the private repo). Consistent verbs, `--json` everywhere,
preview-before-spend, resumability, and install ergonomics (`setup.sh`,
`.env.example`, `gpt doctor`) are **done**.

**Outcome:** best-in-class lightweight CLI for daily WSL use — pure fit-and-finish
over Phases 1–3, no new data semantics.
...
1. **Consistent verb grammar + single entrypoint (FR-U1). — DONE**
...
2. **`--json` on every read command (FR-U2 piping, FR-U3). — DONE**
...
3. **Preview before spend (FR-U2). — DONE**
...
4. **Fast feedback + resumability surfaced (NFR-R2, NFR-R3). — DONE**
...
5. **Install ergonomics + vendored-lib pinning (NFR-Q2, Phase 3→4). — PARTIAL**
...
**Acceptance criteria:** `gpt` exposes a consistent verb set with `--json`
everywhere (FR-U1/U2), shows estimates before spend and state at a glance
(FR-U2/U3), resumes cleanly (NFR-R3), installs on WSL2 in documented steps
(NFR-Q2), and the two repos can no longer silently drift; `pytest -q` is green
(NFR-Q1).
...
- [ ] **Packaging polish (Phase 4 remainder).** Vendored-lib pinning
  (`VENDORED_FROM`) and install ergonomics (one-command setup, version pin).
- [ ] **Publish observability (Phase 3 remainder).** Wire the catalog repo's
