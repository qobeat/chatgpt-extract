...
- **CLI / UX (FR-U1–FR-U3)** — single `gpt` entrypoint with name-driven model
  resolution; preview-before-spend confirmation gate; `gpt info` state at a glance
  with read-only `--json` query commands. *Tests:* `tests/test_confirm.py`,
  `tests/test_store_query.py`, `tests/test_provider_detect.py`.
